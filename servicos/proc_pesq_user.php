...
$result_user = "SELECT * FROM servicos WHERE descricao LIKE '%$usuarios%' OR codigo_produto LIKE '%$usuarios%' LIMIT 20";
...
if(($resultado_user) AND ($resultado_user->num_rows != 0 )){
	while($row_user = mysqli_fetch_assoc($resultado_user)){ ?>
		...
		<tr>
			...
			<td>
				<span class="fw-semibold"><?php echo $row_user['valor']; ?></span>
			</td>
			<td class="text-center text-nowrap fw-medium" style="display: flex; justify-content: center; align-items: center;">
				<a href="vermais_servicos.php?view_servicos=<?php $idCriptografado = base64_encode($row_user['id']); echo $idCriptografado; ?>">
