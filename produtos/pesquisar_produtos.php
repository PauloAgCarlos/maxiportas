...
$result_curso = "SELECT * FROM produtos WHERE codigo_produto LIKE '%$valor_pesquisar%'";
$resultado_curso = mysqli_query($conn, $result_curso);

//Contar o total de produtos
$total_produtos = mysqli_num_rows($resultado_curso);
...
$result_produtos = "SELECT * FROM produtos WHERE codigo_produto LIKE '%$valor_pesquisar%' limit $incio, $quantidade_pg";
...
<header id="page-header">
    ...
    <div id="page-header-search" class="overlay-header bg-header-dark">
          <div class="bg-white-10">
            <div class="content-header">
              <form class="w-100" action="pesquisar_produtos.php" method="GET">
...
<!-- END Header -->
              ...
              <div class="block block-rounded block-mode-loading-refresh">
                <div class="block-header block-header-default">
                  <h3 class="block-title">
                    Resultado da Produtos
                  </h3>
                  <a class="btn btn-alt-secondary" href="visualizar_produtos.php">
                    <i class="fa fa-arrow-left me-1"></i> Visualizar Produtos
                  </a>
                </div>
                <div class="table-responsive">
                  ...
                  <?php
                    ...
                    $selecionar_produtos = new controllers_produtos();
                    $result_produtos = $selecionar_produtos->selecionar_produtos();
                    if(count($result_produtos) > 0)
                    { ?>
                            ...
                            if(count($result_produtos) > 0)
                            {
                              while($row_produtos = mysqli_fetch_assoc($resultado_produtos)){ ?>
                                <tr style="text-align: center;">
                                  <td>
                                      <span class="fw-semibold"><?php echo $row_produtos['codigo_produto']; ?></span>
                                  </td>
                                  <td>
                                    <span class="fs-sm text-muted"><?php echo $row_produtos['descricao_do_produto'];?></span>
                                  </td>
                                  <td>
                                    <?php echo $row_produtos['custo_atual']; ?>
                                  </td>
                                  <td>
                                      <span class="fw-semibold"><?php echo $row_produtos['venda']; ?></span>
                                  </td>
                                  <td class="text-center text-nowrap fw-medium" style="display: flex; justify-content: center; align-items: center;">

                                    <a href="vermais_produtos.php?view_produtos=<?php $idCriptografado = base64_encode($row_produtos['id']); echo $idCriptografado;?>">
                                        <i class="fa fa-eye me-1 opacity-50"></i>
                                    </a>

                                    <?php
                                       echo "<button class='btn' onclick='abrirModal(".$row_produtos['id'].")'><i class='fa fa-fw fa-times text-danger'></i></button>";
                                    ?>
                                  </td>
                                </tr>
                    ...
                    <ul class="pagination">
                        <li>
                            ...
                            if($pagina_anterior != 0){ ?>
                                <a href="pesquisar_produtos.php?pagina=<?php echo $pagina_anterior; ?>&pesquisar=<?php echo $valor_pesquisar; ?>" aria-label="Previous">
                        ...
                        for($i = 1; $i < $num_pagina + 1; $i++){ ?>
                            <li><a href="pesquisar_produtos.php?pagina=<?php echo $i; ?>&pesquisar=<?php echo $valor_pesquisar; ?>"><?php echo $i; ?></a></li>
                        <?php } ?>
                        <li>
                            <?php
                            if($pagina_posterior <= $num_pagina){ ?>
                                <a href="pesquisar_produtos.php?pagina=<?php echo $pagina_posterior; ?>&pesquisar=<?php echo $valor_pesquisar; ?>" aria-label="Previous">
    ...
    <script>
      ...
      document.getElementById('btnConfirmarExclusao').addEventListener('click', function() {
          window.location.href = `eliminar_produtos.php?id=${idProdutoParaExcluir}`;
      });
    </script>
  </body>
</html>
