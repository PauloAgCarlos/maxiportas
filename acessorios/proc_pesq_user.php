...
$result_user = "SELECT * FROM acessorios WHERE descricao LIKE '%$usuarios%' OR codigo_produto LIKE '%$usuarios%' LIMIT 20";
...
if(($resultado_user) AND ($resultado_user->num_rows != 0 )){
	while($row_user = mysqli_fetch_assoc($resultado_user)){ ?>
		...
		<tr>
			...
			<td>
			<?php echo $row_user['custo_unitario']; ?>
			</td>
			<td>
				<span class="fw-semibold"><?php echo $row_user['valor_unitario']; ?></span>
			</td>
			<td class="text-center text-nowrap fw-medium" style="display: flex; justify-content: center; align-items: center;">

				<a href="vermais_acessorios.php?view_acessorios=<?php $idCriptografado = base64_encode($row_user['id']); echo $idCriptografado; ?>">
