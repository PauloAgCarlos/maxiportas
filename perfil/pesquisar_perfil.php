<?php 
    ...
    //Contar o total de perfil
    $total_perfil = mysqli_num_rows($resultado_curso);
    
    //Seta a quantidade de perfil por pagina
    $quantidade_pg = 1;
    
    //calcular o número de pagina necessárias para apresentar os perfil
    $num_pagina = ceil($total_perfil/$quantidade_pg);
    
    //Calcular o inicio da visualizacao
    $incio = ($quantidade_pg*$pagina)-$quantidade_pg;
    
    //Selecionar os perfil a serem apresentado na página
    $result_perfil = "SELECT * FROM perfil WHERE codigo_produto LIKE '%$valor_pesquisar%' limit $incio, $quantidade_pg";
    $resultado_perfil = mysqli_query($conn, $result_perfil);
    $total_perfil = mysqli_num_rows($resultado_perfil);
            ...
            <div class="col-md-12">
              ...
              <div class="block block-rounded block-mode-loading-refresh">
                <div class="block-header block-header-default">
                  ...
                  Resultado da Pesquisa do Perfil
                  </h3>
                  <a class="btn btn-alt-secondary" href="perfil.php">
                    <i class="fa fa-arrow-left me-1"></i> Cadastrar Perfil
                  </a>
                </div>
                <div class="table-responsive">
                  ...
                  <?php
                    ...
                    $selecionar_perfil = new controllers_perfil();
                    $result_perfil = $selecionar_perfil->selecionar_perfil();
                    if(count($result_perfil) > 0)
                    { ?>
                            ...
                            if(count($result_perfil) > 0)
                            {
                              while($row_perfil = mysqli_fetch_assoc($resultado_perfil)){ ?>
                                ...
                                <tr style="text-align: center;">
                                  <td>
                                      <span class="fw-semibold"><?php echo $row_perfil['codigo_produto']; ?></span>
                                  </td>
                                  <td>
                                    <span class="fs-sm text-muted"><?php echo $row_perfil['descricao'];?></span>
                                  </td>
                                  <td>
                                    <span class="fs-sm text-muted"><?php echo $row_perfil['custo_metro'];?></span>
                                  </td>
                                  <td>
                                      <span class="fw-semibold"><?php echo $row_perfil['dimensao']; ?></span>
                                  </td>
                                  <td class="text-center text-nowrap fw-medium" style="display: flex; justify-content: center; align-items: center;">

                                    <a href="vermais_perfil.php?view_perfil=<?php $idCriptografado = base64_encode($row_perfil['id']); echo $idCriptografado;?>">
                                        <i class="fa fa-eye me-1 opacity-50"></i>
                                    </a>

                                    <?php
                                       echo "<button class='btn' onclick='abrirModal(".$row_perfil['id'].")'><i class='fa fa-fw fa-times text-danger'></i></button>";
                                    ?>
                                  </td>
                                </tr>
                  ...
                  <ul class="pagination" style="display: flex; align-items: center; justify-content: space-between; width: 20%; margin: auto;">
                    <li>
                      ...
                      if($pagina_anterior != 0){ ?>
                        <a href="visualizar_perfil.php?pagina=<?php echo $pagina_anterior; ?>" aria-label="Previous">
                    ...
                    for($i = 1; $i < $num_pagina + 1; $i++){ ?>
                      <li><a href="visualizar_perfil.php?pagina=<?php echo $i; ?>"><?php echo $i; ?></a>
                      </li>
                    <?php } ?>
                    <li>
                      <?php
                      if($pagina_posterior <= $num_pagina){ ?>
                        <a href="visualizar_perfil.php?pagina=<?php echo $pagina_posterior; ?>" aria-label="Previous">
