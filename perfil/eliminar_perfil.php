...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['id'])) {
    ...
    $resul = $conection_bd->query("SELECT * FROM perfil WHERE id = $idProduto");
    ...
    try {
        ...
        $stmt = $pdo->prepare("DELETE FROM perfil WHERE id = :id");
        $stmt->bindParam(':id', $idProduto);
        $stmt->execute();
        header("Location: visualizar_perfil.php?eliminado");
        exit;
