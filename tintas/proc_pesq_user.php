...
$result_user = "SELECT * FROM tintas WHERE descricao LIKE '%$usuarios%' OR codigo_produto LIKE '%$usuarios%' LIMIT 20";
...
if(($resultado_user) AND ($resultado_user->num_rows != 0 )){
	while($row_user = mysqli_fetch_assoc($resultado_user)){ ?>
		...
		<tr>
			...
			<td>
			<span class="fs-sm text-muted"><?php echo $row_user['custo'];?></span>
			</td>
			<td class="text-center text-nowrap fw-medium" style="display: flex; justify-content: center; align-items: center;">

				<a href="vermais_tintas.php?view_tintas=<?php $idCriptografado = base64_encode($row_user['id']); echo $idCriptografado; ?>">
