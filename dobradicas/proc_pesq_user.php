...
$result_user = "SELECT * FROM dobradicas WHERE descricao LIKE '%$usuarios%' OR codigo_produto LIKE '%$usuarios%' LIMIT 20";
...
if(($resultado_user) AND ($resultado_user->num_rows != 0 )){
	while($row_user = mysqli_fetch_assoc($resultado_user)){ ?>
		...
		<tr>
			...
			<td>
			<?php echo $row_user['medida_inicial']; ?>
			</td>
			<td class="text-center text-nowrap fw-medium" style="display: flex; justify-content: center; align-items: center;">

				<a href="vermais_dobradicas.php?view_dobradicas=<?php $idCriptografado = base64_encode($row_user['id']); echo $idCriptografado; ?>">
