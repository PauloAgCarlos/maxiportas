...
$result_user = "SELECT * FROM agregar WHERE descricao LIKE '%$usuarios%' LIMIT 20";
...
if(($resultado_user) AND ($resultado_user->num_rows != 0 )){
	while($row_user = mysqli_fetch_assoc($resultado_user)){ ?>
		...
		<tr>
			...
			<td>
				<?php echo $row_user['ultima_alteracao']; ?>
			</td>
			<td class="text-center text-nowrap fw-medium" style="display: flex; justify-content: center; align-items: center;">
				<a href="vermais_agregar.php?view_agregar=<?php $idCriptografado = base64_encode($row_user['id']); echo $idCriptografado; ?>">
